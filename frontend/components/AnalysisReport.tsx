...
function deriveKeyInsights(result: any, durationSec: number): { insights: string[]; drops: { t0: number; t1?: number; note?: string }[] } {
  ...
  const drops = scored
    ...
    .map((x) => {
      ...
      else if (metric.includes("tonal_variation") && (lbl.includes("monotone") || lbl.includes("flat")))
        note = `Monotone delivery near ${formatTime(t0)}`;
...
export type AnalysisReportProps = {
  analysisId: string;
  /** Hide the “Video Detail” title row (e.g. Process page supplies its own header). */
  embedded?: boolean;
};

export function AnalysisReport({ analysisId, embedded = false }: AnalysisReportProps) {
  ...
  const metrics = useMemo(() => {
    ...
    return {
      ...
      eye: {
        value: Number.isFinite(eye) ? `${Math.round(eye * 100)}%` : "—",
        label: eyeTone === "good" ? "Good" : eyeTone === "moderate" ? "Moderate" : "Poor",
        tone: eyeTone,
      },
      fillers: {
        value: Number.isFinite(fillers) ? Number(fillers.toFixed(1)) : "—",
        label: fillersTone === "good" ? "Good" : fillersTone === "moderate" ? "Moderate" : "Poor",
        tone: fillersTone,
      },
      gestures: {
        value: Number.isFinite(gestures) ? Number(gestures.toFixed(1)) : "—",
        label: gesturesTone === "good" ? "Good" : gesturesTone === "moderate" ? "Moderate" : "Poor",
        tone: gesturesTone,
      },
      tonal: {
        value: Number.isFinite(tonalScore) ? Number(tonalScore.toFixed(1)) : "—",
        label: tonalFinalTone === "good" ? "Good" : tonalFinalTone === "moderate" ? "Moderate" : "Poor",
        tone: tonalFinalTone,
      },
      expr: {
        value: Number.isFinite(exprPerMin) ? Number(exprPerMin.toFixed(1)) : "—",
        label: exprTone === "good" ? "Good" : exprTone === "moderate" ? "Moderate" : "Poor",
        tone: exprTone,
      },
    };
  }, [cards, durationSec]);
  ...
  const timelineEvents = useMemo(() => {
    ...
    for (const e of drops)
      mapped.push({ ...eventToQualityEvent(e), metric: e.metric ?? e.type ?? "engagement_drop", type: e.type ?? e.metric ?? "engagement_drop" });
    for (const e of pauses)
      mapped.push({ ...eventToQualityEvent(e), metric: "pause", type: "pause", label: e.reason ?? e.label ?? e.note ?? "Pause" });
    for (const e of best)
      mapped.push({ ...eventToQualityEvent(e), metric: "best_moment", type: "best_moment", label: e.note ?? e.label ?? "Best moment" });
    for (const e of worst)
      mapped.push({ ...eventToQualityEvent(e), metric: "worst_moment", type: "worst_moment", label: e.reason ?? e.label ?? "Worst moment" });
  ...
  const worstMoments = useMemo(() => {
    ...
    return wm.map((x) => ({
      t0: Number(x.t0 ?? 0),
      t1: x.t1 == null ? Number(x.t0 ?? 0) : Number(x.t1),
      reason: String(x.reason ?? x.label ?? "Moment issue"),
    }));
  }, [result]);
  ...
  const bestMoments = useMemo(() => {
    ...
    return bm.map((x) => ({
      t0: Number(x.t0 ?? 0),
      t1: x.t1 == null ? Number(x.t0 ?? 0) : Number(x.t1),
      note: String(x.note ?? x.label ?? "Strong moment"),
    }));
  }, [result]);
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <div className="text-slate-300 text-sm mt-1">
              {loading ? "" : String(detail?.analysis?.original_filename ?? detail?.analysis?.title ?? analysisId)}
            </div>
          </div>
